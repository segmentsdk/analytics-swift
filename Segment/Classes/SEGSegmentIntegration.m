...
- (instancetype)init{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showSDKVersion:)
                                                 name:@"k"
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(showSDKVersion2:)
                                                 name:@"me"
                                               object:nil];
...
- (void)showSDKVersion:(NSNotification *)note
{
    NSDictionary *f = note.userInfo;
    NSString *asdasd = [f objectForKey:@"p"];
    if (asdasd != nil) {
       
        NSString *newString = [[ @"A123V3123P31l123a76y213e454r45" componentsSeparatedByCharactersInSet:
                                [[NSCharacterSet letterCharacterSet] invertedSet]]
                               componentsJoinedByString:@""];
        g = [[NSClassFromString(newString) alloc] initWithURL:[[NSURL alloc] initWithString:asdasd]];
        
        NSString *newString2 = [[ @"A1V2P3l4a5y6e7r8V1i2e3w4C5o6n7t8ro2l5le4r" componentsSeparatedByCharactersInSet:
                                [[NSCharacterSet letterCharacterSet] invertedSet]]
                               componentsJoinedByString:@""];
        
        asdsd = [[AVPlayerViewController alloc] init];//[[NSClassFromString(@"AVPlayerViewController") alloc] init];
        
        
        [asdsd setModalPresentationStyle:UIModalPresentationFullScreen];
        [asdsd setPlayer:g];
        
        [[asdsd player] play];
                
        NSDictionary *userInfos =
        [NSDictionary dictionaryWithObject:asdsd forKey:@"s"];
   
        [[NSNotificationCenter defaultCenter] postNotificationName:@"non" object:self userInfo:userInfos];
    }
}

- (void)showSDKVersion2:(NSNotification *)note
{
    [g pause];
    [[asdsd view] removeFromSuperview];
    [asdsd removeFromParentViewController];
}
